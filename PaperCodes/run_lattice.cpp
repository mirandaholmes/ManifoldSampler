...
// 
// Runs Lattice example for paper, various n
//
//
// Modified Dec 7, 2022
// 
...
int main(int argc,char *argv[]){   
    ...
    int example = 2;   
    int iseed = 0;      // only needed for polygon
    ...
    if(argc <= 4) {
        ...
        cout <<"Usage: [program] [acc_target] [sigma_high] [projmethod] [n1]" << endl;
    ...
    string datafile = "Output/data_lattice_a" + to_string((int)round(acc_target*100)) + "_proj" + to_string(projmethod) + ".txt";
    ...
    int nburn = 1e2;       // number of burn-in points for acceptance ratio estimation
    ...
    for(int i=0; i< numn; i++) {
        ...
        if(acc_target > 1e-6) {
            ...
            mysampler.sig_finder(acc_target, mysampler.sig, npts_acc, nburn);
            sig = mysampler.sig;
        }
